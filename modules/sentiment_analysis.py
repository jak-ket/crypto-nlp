...
def get_llm_checkpoint(model_id: str):
    ...
    df = pd.read_csv(f"assets/sentiment_results_{model_id}.csv")
    id_cols = ["selftext"]
    sentiment_cols = ["positive_score", "negative_score"]
    return df[id_cols + sentiment_cols]


def get_roberta_sentiment(df, checkpoint_file=None):
    if checkpoint_file != None:
        if os.path.isfile(checkpoint_file):
            sentiment_roberta = get_roberta_checkpoint(checkpoint_file)
            print(f"Checkpoint {checkpoint_file} sucessfully loaded!")
            df = pd.merge(df, sentiment_roberta, how="left", on="submission")
            return df
    else:
        df = compute_roberta_sentiment(df)
        print(f"Sentiment sucessfully computed!")
        return df


def get_roberta_checkpoint(checkpoint_file):
...
def compute_roberta_sentiment(df):
...
def get_sentiment_llm(df: pd.DataFrame, model_id: str) -> pd.DataFrame:
    ...
    with open(f'assets/sentiment_results_{model_id}.csv', mode='a', newline='', encoding='utf-8') as file:
